This is the blog. It renders the markdown files under `blog/` the same way
...
- Each numbered folder in `blog/` (like `00-welcome`) is a section in the
  sidebar. Each `.md` file inside it is a page.
- The page title comes from the `.md` filename, rendered as the page's heading.
...
To add a post, drop a new `.md` file under a folder in `blog/`.
